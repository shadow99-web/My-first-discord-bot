...
async function runEmojiSearch(interactionOrMessage, keyword, isSlash = true) {
  ...
  const generateEmbed = (pageIndex) => {
    ...
    emojis.forEach((item, idx) => {
      const media = item.preview_url; // Lottie preview URL (GIF/MP4)
  ...
  const replyFunc = isSlash ? interactionOrMessage.reply.bind(interactionOrMessage) : interactionOrMessage.channel.send.bind(interactionOrMessage.channel);
  ...
  collector.on('collect', async i => {
    const userId = isSlash ? i.user.id : i.user.id;
...
module.exports = {
  data: {
    name: 'emoji',
    description: 'Search for emoji animations',
    options: [{ name: 'search', type: 3, description: 'Keyword to search', required: true }]
  },
